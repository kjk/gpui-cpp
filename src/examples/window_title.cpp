...
static El* OnRender(AppHost* host, Arena* frame, WinSize size) {
    ...
    El* bar = Div(frame)
                  ...
                  ->Child(TextEl(frame, StrL("App with Custom title bar"))->Font(14)->Fg(th.foreground))
                  ->Child(TextEl(frame, StrL("Right Item"))->Font(14)->Fg(th.mutedFg));
    ...
    return Div(frame)->FlexCol()->SizeFull()->Bg(th.background)->Child(bar)->Child(Div(frame)->H(1)->W(kFill)->Bg(th.titleBarBorder))->Child(body);
}
