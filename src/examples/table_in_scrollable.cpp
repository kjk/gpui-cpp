...
static float PageContentH() {
    return kPagePad + kAboveH + kPageGap + kTableH + kPageGap + kBelowH + kPagePad;
}
...
static El* OnRender(AppHost* host, Arena* frame, WinSize size) {
    ...
    El* table = Div(frame)->FlexCol()->H(kTableH)->W(kFill)->Border(1, th.border)->Radius(th.radius)->Bg(th.tableBg);
    El* head = Div(frame)->FlexRow()->H(kHeadH)->Shrink0()->Bg(th.muted)->BorderB(1, th.tableRowBorder);
    for (int i = 0; i < 5; i++) {
        head->Child(Div(frame)->W(widths[i])->H(kHeadH)->PadX(8)->ItemsCenter()->Child(
            TextEl(frame, Str(heads[i]))->Font(12)->Fg(th.tableHeadFg)));
    }
    table->Child(head);

    El* body = Div(frame)->FlexCol()->Grow()->W(kFill)->ClipY()->ScrollY(app->tableScroll);
    for (int r = 0; r < kRows; r++) {
        ...
        for (int c = 0; c < 5; c++) {
            row->Child(Div(frame)->W(widths[c])->H(kRowH)->PadX(8)->ItemsCenter()->Child(
                TextEl(frame, Str(cells[c]))->Font(13)->Fg(th.foreground)));
        }
        body->Child(row);
    }
    table->Child(body);
    ...
    float tThumbY = maxT > 0 ? (app->tableScroll / maxT) * (bodyH - tThumbH) : 0;
    table->Child(Thumb(frame, kHeadH + tThumbY, tThumbH, RgbaOpacity(th.mutedFg, 0.55f)));
    ...
    El* root = Div(frame)->SizeFull()->ClipY()->ScrollY(app->pageScroll)->Bg(th.background);
    root->Child(page);
