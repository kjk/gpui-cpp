...
El* ShowcaseDatePicker(ShowcaseApp* app, Arena* a) {
    El* trigger = Button::New(a, StrL("date-trigger"), ClickDate)
                      ->W(250)
                      ->H(28)
                      ->PadX(12)
                      ->ItemsCenter()
                      ...
                      ->Child(TextEl(a, StrL("Aug 12, 2026"))->Font(12)->Fg(Rgb(0x17, 0x17, 0x17)))
    ...
    return DatePicker::New(a, StrL("example-date-picker"))
        ->W(250)
        ->Child(Popup::New(a, StrL("date-picker-popup"), trigger)->Content(cal)->IntoEl());
}
