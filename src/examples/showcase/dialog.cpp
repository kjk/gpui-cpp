...
enum { ClickDlgOpen = 350, ClickDlgCancel = 351, ClickDlgSave = 352, ClickDlgField = 353 };
...
El* ShowcaseDialog(ShowcaseApp* app, Arena* a) {
    ...
    root->Child(Button::New(a, StrL("open-dialog"), ClickDlgOpen)
                    ...
                    ->JustifyCenter()
                    ->Bg(Rgb(0, 0, 0))
                    ->Child(TextEl(a, StrL("Edit profile"))->Font(12)->Fg(Rgb(0xff, 0xff, 0xff))));
    if (!app->dialogOpen) {
        return root;
    }
    El* panel = Div(a)
                    ...
                    ->Child(DialogTitle::New(a)->Child(TextEl(a, StrL("Edit profile"))->Font(12)->Fg(Rgb(0x17, 0x17, 0x17))->Semibold()))
                    ->Child(DialogDescription::New(a)->Child(
                        TextEl(a, StrL("Update the public details shown on your profile."))
                            ->Font(12)
                            ->Fg(Rgb(0x73, 0x73, 0x73))
                            ->Wrap()
                            ->MaxW(264)))
                    ->Child(Div(a)->PadT(12)->Child(TextEl(a, StrL("Display name"))->Font(14)->Fg(Rgb(0x17, 0x17, 0x17))))
                    ...
                    ->Child(Div(a)
                                ...
                                ->Child(DialogClose::New(a, ClickDlgCancel)
                                            ->Child(Button::New(a, StrL("dialog-cancel"), ClickDlgCancel)
                                                        ->H(28)
                                                        ->PadX(12)
                                                        ->ItemsCenter()
                                                        ->Border(1, Rgb(0xd4, 0xd4, 0xd4))
