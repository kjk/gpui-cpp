...
enum { ClickTab0 = 520, ClickTab1 = 521, ClickTab2 = 522 };

El* ShowcaseTabs(ShowcaseApp* app, Arena* a) {
    const char* labels[] = {"Overview", "Activity", "Settings"};
    El* bar = Tabs::New(a, StrL("example-tabs"))->FlexRow()->PadX(8)->PadT(4)->BorderB(1, Rgb(0xd4, 0xd4, 0xd4));
    for (int i = 0; i < 3; i++) {
        bool on = app->tab == i;
        El* tab = Tab::New(a, DupFmt(a, "tab-%d", i), ClickTab0 + i)
                      ->H(28)
                      ->PadX(8)
                      ->ItemsCenter()
                      ->BorderB(2, on ? Rgb(0x17, 0x17, 0x17) : Rgb(0xff, 0xff, 0xff))
                      ->HoverBg(Rgb(0xf5, 0xf5, 0xf5));
        El* lab = TextEl(a, Str(labels[i]))->Font(12)->Fg(Rgb(0x17, 0x17, 0x17));
        if (on) {
            lab->Semibold();
        }
        tab->Child(lab);
    ...
    return Div(a)
        ...
        ->Child(Div(a)
                    ...
                    ->Child(TextEl(a, Str(sub))->Font(12)->Fg(Rgb(0x73, 0x73, 0x73))->Wrap()->MaxW(260)));
}
