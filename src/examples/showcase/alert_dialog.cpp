...
enum { ClickAlertOpen = 210, ClickAlertCancel = 211, ClickAlertOk = 212 };
...
El* ShowcaseAlertDialog(ShowcaseApp* app, Arena* a) {
    ...
    root->Child(Button::New(a, StrL("open-alert-dialog"), ClickAlertOpen)
                    ...
                    ->JustifyCenter()
                    ->Bg(Rgb(0, 0, 0))
                    ->Child(TextEl(a, StrL("Delete project"))->Font(12)->Fg(Rgb(0xff, 0xff, 0xff))));
    if (!app->alertOpen) {
        return root;
    }
    ...
    El* panel = Div(a)
                    ...
                    ->Child(AlertDialogTitle::New(a)->Child(TextEl(a, StrL("Delete project?"))->Font(14)->Fg(Rgb(0x17, 0x17, 0x17))))
                    ->Child(Div(a)->H(8))
                    ->Child(AlertDialogDescription::New(a)->Child(
                        TextEl(a, StrL("This permanently deletes Acme Studio and all of its data."))
                            ->Font(12)
                    ...
                    ->Child(Div(a)
                                ...
                                ->Child(AlertDialogAction::New(a)->Child(
                                    Button::New(a, StrL("confirm-delete"), ClickAlertOk)
                                        ...
                                        ->Child(TextEl(a, StrL("Delete"))->Font(12)->Fg(Rgb(0xff, 0xff, 0xff))))));
    ...
    El* popup = AlertDialogPopup::New(a)
                    ...
                    ->JustifyCenter()
                    ->Child(panel);
    root->Child(AlertDialog::New(a)->Backdrop(backdrop)->Popup(popup)->IntoEl());
    return root;
}
