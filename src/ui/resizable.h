...
struct ResizableState {
    ...
    Vec<float> mins;
    Vec<float> maxs;
    // A panel with no size of its own takes what is left. -1 is none, which
    // is what a group of fixed panels is.
    int growIx = -1;
...
struct Resizable {
    ...
    // The panel that takes what the others leave. Rust's `grow()`.
