...
// scales, named without a component in sight — no `button`, no `table`, no
// `sidebar`. Rust keeps them on `gpui_base::Theme` and the `crates/ui` theme
// projects them out of its own palette every time it is applied; the same
// pair of functions is below. They live here rather than in `src/base`
// because `Theme` does, and `ThemeTokens` above already has the name Rust's
// ui layer uses for its own (gradient-carrying) set.
//
// One thing in this tree reads them, which is one more than in Rust: the
// scrollbar's `radius.md`.
