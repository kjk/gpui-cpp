...
El* ShellRoot::Render(ShellRoot* self, Ctx* cx) {
    ...
    if (self->fpsHudVisible) {
        ...
        if (slot) {
            ...
            if (FpsMonitor* monitor = slot->Get(cx->app)) {
                monitor->continuous = self->fpsHud.continuous;
                if (self->fpsHud.hasFrameBudget) {
                    monitor->frameBudget = self->fpsHud.frameBudget;
                    monitor->axisMax = self->fpsHud.frameBudget * 2.f;
                }
            }
            hud = FpsOverlayEl(cx, *slot, self->fpsHud.anchor);
        }
    }
