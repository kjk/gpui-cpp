...
## Shell root, assets, accessibility and dock integration

The application host is now a real `ShellRoot`: script views render inside the
same Root, dialog, sheet and toast infrastructure as native views, and nested
views participate in entity ownership and invalidation. Script assets resolve
through a canonical application-root provider with traversal and symlink
escape checks. Shell accessibility roles, names, descriptions, selected and
disabled state materialize into the native accessibility tree. Dock panels
retain script views, persist their layout, and enter the upstream Layout scope
while asking JavaScript for panel chrome.

Tests cover root overlays, asset authority, all published accessibility roles,
dock persistence and layout-scope restrictions. MSVC release passes 21,373
checks.

## Shell plugin manifests and manager

`gpui-shell.json` is parsed with the upstream schema and strict unknown-field,
identity, entry-path and capability validation. Discovery is deterministic,
bounded and symlink-free across configured plugin directories. Loading asks a
host authorization callback before constructing the view, gives each plugin
its frozen policy, host modules and per-user storage directory, and unloading
drops the complete entity/runtime-owned subtree. A changed manifest replaces
the prior discovery record without mutating a loaded plugin behind the host's
back.

Tests cover discovery, malformed manifests, denial before execution,
capability projection, loading, lookup and unload. MSVC release passes 21,402
checks.

## Shell canonical TypeScript declarations

The tracked declaration payload is generated byte-for-byte from the pinned
upstream `cargo run -p gpui-shell -- types` output. `cmd/update-shell-types.ts`
regenerates it or checks for drift. The runtime appends registered host-module
declarations, writes `gpui.d.ts` at the application root and at bounded nested
import directories, leaves identical files untouched, and refuses symlinked
destinations. The result is the full upstream 642,527-byte API rather than a
hand-maintained subset.

Tests cover the canonical payload, synchronous and asynchronous host exports,
nested writers and idempotence. MSVC release passes 21,427 checks.

## Shell command and upstream todo application

`gpui_shell` is a desktop console target with run, `check` and `types` modes,
`--watch`, and development-mode dynamic-code opt-in. It accepts either an
application directory or its entry file, applies a deny-first local policy or
the optional plugin manifest, installs root-relative assets, and stores each
application under a stable path-derived identity in the user's data home.
`cmd/run.ts` passes arguments after `--` unchanged and waits for console targets,
and the dist snapshot carries both the command and the script example.

The upstream `examples/js_todolist` files are copied unchanged. To run them,
the runtime completed the source-compatible element `map`/`when` helpers,
generation-bound frozen `cx.theme()` snapshots, `set_theme` with all semantic
color/spacing/radius validation, and HTTP(S)-only `cx.open_url`. Theme state is
read from Base's semantic global, preserving scales that have no legacy UI
palette slot.

QuickJS-NG remains exactly one generated `.h` and one generated `.c` under
`src/quickjs`, pinned by SHA in `cmd/run.ts`. Its updater strips C comments,
trailing horizontal whitespace and repeated blank lines, preserves required
license notices as inert string literals, and verifies that the generated
files round-trip unchanged.

The standing repository boundaries remain explicit: network access is the
existing bounded GET only, so upstream POST/OAuth requests, raw TCP and
WebSockets are excluded; `gpui_shell` has no wasm target; pattern and checkerboard
backgrounds remain solid until the native renderer has a repeating-pattern
