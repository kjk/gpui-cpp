...
static const char* kMsgs[] = {
    ...
    "Sure — drag from anywhere, even from the blank space between bubbles, then press Ctrl+C to copy everything.",
...
static void OnInit(AppHost* host) {
    ...
    strncpy_s(app->in.placeholder, "Type here (selection must NOT start from here)", _TRUNCATE);
    host->input = &app->in;
    app->copied[0] = 0;
    app->selFrom = -1;
    app->selTo = -1;
...
static El* Bubble(Arena* a, int ix, const Theme& th) {
    ...
    El* inner = Div(a)
                    ...
                    ->Click(10 + ix)
                    ->Child(TextEl(a, Str(kMsgs[ix]))->Font(14)->Fg(th.foreground)->Wrap());
...
static El* OnRender(AppHost* host, Arena* frame, WinSize size) {
    ...
    El* col = Div(frame)->FlexCol()->SizeFull()->Pad(16)->Gap(12)->Bg(th.background);
    ...
    Str shown = app->in.len > 0 ? Str(app->in.buf, app->in.len) : Str(app->in.placeholder);
    col->Child(Div(frame)
                   ->W(kFill)
                   ->H(36)
                   ->PadX(12)
                   ...
                   ->Border(1, th.border)
                   ->Click(2)
                   ->Child(TextEl(frame, shown)->Font(14)->Fg(app->in.len ? th.foreground : th.mutedFg)));
    return col;
}
