...
El* LabelRender(StoryApp* app, Arena* a) {
    ...
    El* sec = StorySection(a, "Default", "Primary text with an optional secondary hint.");
    El* col = Div(a)->FlexCol()->Gap(8);
    col->Child(component::Label::New(a, StrL("Display name"))->IntoEl());
    col->Child(component::Label::New(a, StrL("Email"))->Secondary(StrL("optional"))->IntoEl());
    col->Child(component::Label::New(a, StrL("API token"))->Masked(true)->IntoEl());
