...
El* HoverCardRender(StoryApp* app, Arena* a) {
    ...
    if (app->hoverId) {
        card = Div(a)
                   ...
                   ->Bg(th.background)
                   ->Radius(th.radius);
        card->Child(StoryTxt(a, StrL("gpui-base"), 14, th.foreground)
                        ->Semibold());
        card->Child(
            StoryTxt(a,
                     StrL("Unstyled primitives for building design systems."),
                     12, th.mutedFg)
                ->Wrap()
                ->MaxW(220));
    }
    StorySectionAdd(sec, component::HoverCard::New(a)
                             ->Trigger(trigger)
                             ->Content(card)
                             ->Open(app->hoverId != 0)
                             ->IntoEl());
