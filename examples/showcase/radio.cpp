...
static El* RadioRow(Arena* a, Str id, int clickId, bool on, const char* title,
                    const char* sub, bool disabled) {
    El* row = Radio::New(a, id, disabled ? 0 : clickId)
                  ->FlexRow()
                  ->ItemsStart()
                  ->Gap(8);
...
El* ShowcaseRadioGroup(ShowcaseApp* app, Arena* a) {
    return RadioGroup::New(a, StrL("example-radio-group"))
        ...
        ->FlexCol()
        ->Gap(8)
        ->Child(RadioRow(a, StrL("example-radio"), ClickRadioStd,
                         app->radioSel == 0, "Standard", "3–5 business days",
                         false))
        ->Child(RadioRow(a, StrL("express-radio"), ClickRadioExpress,
                         app->radioSel == 1, "Express", "Next business day",
                         false))
        ->Child(RadioRow(a, StrL("pickup-radio"), 0, false, "Local pickup",
